...
def part1_show_T_pose(viewer, joint_names, joint_parents, joint_offsets):
    ...
    for joint_idx, parent_idx in enumerate(joint_parents):
        ...
        ########## Code Start ############
        if parent_idx != -1 :
            #joint_offsets[joint_idx] = joint_offsets[joint_idx] - joint_offsets[parent_idx]
            global_joint_position[joint_idx] =  joint_offsets[joint_idx] + global_joint_position[parent_idx]
        if parent_idx == -1 :
...
def part2_forward_kinametic(viewer, joint_names, joint_parents, joint_offsets, joint_positions, joint_rotations, show_animation=False):
    ...
    ########## Code Start ############
    for joint_idx, parent_idx in enumerate(joint_parents):
        r1 = R.from_quat(global_joint_orientations[:, parent_idx, :])
        r2 = R.from_quat(joint_rotations[:, joint_idx, :])
        rotated_offset = r1.apply(joint_positions[:, joint_idx, :])
        global_joint_positions[:, joint_idx, :] = global_joint_positions[:, parent_idx, :] + rotated_offset
        global_joint_orientations[:, joint_idx, :] = (r1 * r2).as_quat()
